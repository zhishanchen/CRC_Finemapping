library(dplyr)
colomics <- read.table("result-colomics/Trans_in_Colonomics_MN_mQTL_ALL_output.txt",header=T,sep="\t",row.names=NULL)
gtex <- read.table("result-gtex/Trans_in_GTEx_mQTL_ALL_output.txt.rsid",header=F,sep="\t",row.names=NULL)
...
write.table(df,"Trans_ALL_mQTLs.txt",quote=F,sep="\t",row.names=F)
