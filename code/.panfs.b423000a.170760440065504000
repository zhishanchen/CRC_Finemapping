...
chr=`grep "$region\b" /gpfs52/nobackup/sbcs/chenz27/fine-mapping6/risk_variants/07302022/CRC_consortium_ALL_risk_SNPs.txt_latest.bed.merge | cut -f 1`

trans="Trans_indepSNP.txt"
eur="Eur_indepSNP.txt"
asian="Asian_indepSNP.txt"
report="reported_SNP.txt"
...
do
    ...
    do
	echo $b
	plink2 --bfile /scratch/sbcs/chenzs/New_Public_data/1KG/Eur/bychr/1kg.$chr.phase3.20130502.Eur --ld $a $b | grep "\^" > tmp/$region.out
	...
	plink2 --bfile /scratch/sbcs/chenzs/New_Public_data/1KG/asian/1kg.$chr.phase3.20130502.asian --ld $a $b | grep "\^" > tmp/$region.out
	line=`wc -l tmp/$region.out |awk '{print $1}'`
	if [[ $line = 1 ]]; then
